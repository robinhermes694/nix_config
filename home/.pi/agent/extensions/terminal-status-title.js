// Sets the terminal window title to a spinner while Pi is working, then a
// completion mark with the session name or current directory.
//
// Copyright (c) 2026 Kun Chen.
// MIT Licensed - see the LICENSE file in home/.pi/agent/extensions/calm/ for the permissive license text.
//
// This extension is a standalone JavaScript file (no TypeScript compilation
// needed) and is loaded by Pi's global extensions directory symlink.
// It does not import any Pi modules at module scope; it only uses the `pi`
// global that Pi injects at runtime.
...
function getSessionName(pi) {
  const name = pi.getSessionName ? () => pi.getSessionName() : undefined;
  return typeof name === "function" ? name().trim() : "";
}
